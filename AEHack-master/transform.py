...
def convert_to_elv_az(latTar, lngTar, altTar):
  ...
  elevation = math.acos((x*dx + y*dy + z*dz) / math.sqrt((x**2+y**2+z**2)*(dx**2+dy**2+dz**2)))
  # print(math.degrees(elevation))

  azimuth_cos = (-z*x*dx - z*y*dy + (x**2+y**2)*dz) / math.sqrt((x**2+y**2)*(x**2+y**2+z**2)*(dx**2+dy**2+dz**2))
  azimith_sin = (-y*dx + x*dy) / math.sqrt((x**2+y**2)*(dx**2+dy**2+dz**2))
